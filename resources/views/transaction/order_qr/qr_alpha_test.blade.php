...
<body>
    ...
    $row_count = round(($paper_width - 36)/($border_size+5));
    ...
    //
    // $col_count = round(count($qrcode)/ $row_count);
    // if($qr_size>=53 && $qr_size<=91){
    //     $row_count += 1;
    // };
    ...
    <?php if ($counter % ($row_count * $col_count)==0) { ?>
        <div style="background-color: darkseagreen;height: {{$header_footer_height}}mm;">
    ...
    <table style="background-color: burlywood;padding-left: {{$margin_left_right}}mm;padding-right: {{$margin_left_right}}mm;">
        <?php } ?>
        <?php if ($counter % $row_count == 0) { ?>
        <tr>
            <?php } ?>
            <td class="qr"
                style="background-color: blueviolet;width: {{$border_size}}mm;height:{{$qr_height}}mm">
                <div class="row">
                    <div
                        style="background-color: chartreuse;text-align:center;padding-top:{{ $padding_height }}px;padding-bottom: {{ $padding_height }}px">
                        ...
                        <figcaption
                            ...
                            A {{$qr_height." ".$border_size." ".$row_count." ".$col_count." /".$qr_size."/".$paper_width}}
                        </figcaption>
                    </div>
                    <div style="background-color: darkmagenta;" class="image_container">
                        <img src="{{ asset('border_alpha.jpeg')}}" height="{{ $qr_size / 2.6 }}px"
            ...
            <?php if (($counter + 1) % $row_count != 0) { ?>
                <td style="background-color: cyan;width:4mm;">
                </td>
            <?php } ?>
        ...
        <?php if ($counter % ($row_count * $col_count) != 0) { ?>
        <tr>
            <td style="background-color: darkgreen;height: {{$qr_vertical_margin}}mm;">
            </td>
        </tr>
    ...
    <div style="background-color: darkseagreen;height: 4mm;">
    </div>
