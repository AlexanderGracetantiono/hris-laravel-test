...
@section('page_title', 'Customer Scan Log')
...
<script src="{{ asset('custom/js/home/location_zeta.js') }}"></script>
...
<div class="row">
    <div class="col-xl-12">
        <div class="card card-custom gutter-b">
            ...
            <form class="form" id="form" method="GET" action="{{ route('dashboard_scan_zeta') }}" data-form-success-redirect="{{ route('master_data_batch_production_view') }}"  enctype="multipart/form-data">
                <div class="card-body">
                    ...
                    <table class="table table-bordered table-checkable" id="kt_datatable1" >
                        ...
                        <tbody>
                            @foreach($data as $data)
                                <tr>
                                    <td>{{ $data["LGPRT_QR_CODE"] }}</td>
                                    <td>{{ $data["LGPRT_CST_SCAN_TEXT"] }}</td>
                                    <td>{{ $data["LGPRT_CST_SCAN_TIMESTAMP"] }}</td>
                                    <td><a target="_blank" href="http://maps.google.com/?q={{ $data['LGPRT_CST_SCAN_LAT'] }},{{ $data['LGPRT_CST_SCAN_LNG'] }}">{{ $data["LGPRT_CST_SCAN_LAT"] }} , {{ $data["LGPRT_CST_SCAN_LNG"] }}</a></td>
                                    <td>{{ $data["LGPRT_MPRCAT_TEXT"] }}</td>
                                    <td>{{ $data["LGPRT_MPRDT_TEXT"] }}</td>
                                    <td>{{ $data["LGPRT_MPRMO_TEXT"] }}</td>
                                    <td>{{ $data["LGPRT_MPRVE_TEXT"] }}</td>
