...
class PartialQrController extends Controller
{
	public function index(Request $request)
	{
		...
		elseif ($qr_type["type"] == 2) {
			...
			$insert_lgprt_data = [
				"LGPRT_QR_CODE" =>  $request["code"],
				"LGPRT_TYPE" =>  $qr_type["type"],
				"LGPRT_MCOMP_CODE" =>  $qr_type["data"]["TRQRA_MCOMP_CODE"],
				"LGPRT_MCOMP_NAME" =>  $qr_type["data"]["TRQRA_MCOMP_TEXT"],
				"LGPRT_MBRAN_CODE" =>  $qr_type["data"]["TRQRA_MBRAN_CODE"],
				"LGPRT_MBRAN_NAME" => $qr_type["data"]["TRQRA_MBRAN_TEXT"],
				"LGPRT_MPRCAT_CODE" =>$qr_type["data"]["TRQRA_MPRCA_CODE"],
				"LGPRT_MPRCAT_TEXT" =>$qr_type["data"]["TRQRA_MPRCA_TEXT"],
				"LGPRT_MPRDT_CODE" => $qr_type["data"]["TRQRA_MPRDT_CODE"],
				"LGPRT_MPRDT_TEXT" => $qr_type["data"]["TRQRA_MPRDT_TEXT"],
				"LGPRT_MPRMO_CODE" => $qr_type["data"]["TRQRA_MPRMO_CODE"],
				"LGPRT_MPRMO_TEXT" => $qr_type["data"]["TRQRA_MPRMO_TEXT"],
				"LGPRT_MPRVE_CODE" => $qr_type["data"]["TRQRA_MPRVE_CODE"],
				"LGPRT_MPRVE_TEXT" => $qr_type["data"]["TRQRA_MPRVE_TEXT"],
				"LGPRT_CST_SCAN_BY" => $request["scan_by"],
				"LGPRT_CST_SCAN_TEXT" => $request["scan_text"],
				"LGPRT_CST_SCAN_TIMESTAMP" => date("Y-m-d H:i:s"),
				"LGPRT_CST_SCAN_LAT" =>$request["lat"],
				"LGPRT_CST_SCAN_LNG" =>$request["long"],
				"LGPRT_CST_SCAN_COUNTRY" =>$request["country"],
				"LGPRT_CST_SCAN_PROVINCE" =>$request["province"],
				"LGPRT_CST_SCAN_REGENCY" =>$request["regency"],
				"LGPRT_CST_SCAN_DISTRICT" =>$request["district"],
				"LGPRT_CST_SCAN_ADDRESS" =>$request["address"],
				"LGPRT_CST_SCAN_DEVICE_ID" =>$request["device_id"],
				"LGPRT_CST_SCAN_APP_VERSION" =>$request["app_version"],
				"LGPRT_CREATED_BY" => $request["scan_by"],
				"LGPRT_CREATED_TEXT" => $request["scan_text"],
				"LGPRT_CREATED_TIMESTAMP" => date("Y-m-d H:i:s"),
			];
			$insert_lgprt = std_insert([
				"table_name" => "LGPRT",
				"data" => $insert_lgprt_data
			]);
		}
		return response()->json([
			...
			$check_exists_masco = std_get([
				...
				"where" => [
					[
						...
						"operator" => "=",
						"value" => $check_exists_zeta["TRQRZ_MASCO_CODE"],
					],
				],
				"first_row" => true
			]);
			$data_qr_alpha = std_get([
				...
				"where" => [
					[
						"field_name" => "TRQRA_CODE",
						"operator" => "=",
						"value" => $check_exists_masco["MASCO_TRQAH_CODE"],
					],
				],
				"first_row" => true
			]);
