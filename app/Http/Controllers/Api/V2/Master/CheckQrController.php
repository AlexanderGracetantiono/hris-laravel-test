...
class CheckQrController extends Controller
{
	...
	public function check_production_qr($request)
	{
		...
		if ($check_exists["TRQRA_MBRAN_CODE"] != $data_batch["MABPR_MBRAN_CODE"]) {
			$reponse = "E21";
			return $reponse;
		}
	...
	public function check_packaging_qr($request)
	{
		...
		if ($check_exists["TRQRZ_MBRAN_CODE"] != $data_batch["SUBPA_MBRAN_CODE"]) {
			$reponse = "E21";
			return $reponse;
		}
	...
	public function check_sticker_code($request)
	{
		...
		if ($request["employee_role"] == "6") {
			...
			if ($check_sticker_paired["MASCO_MBRAN_CODE"] != $data_batch["MABPR_MBRAN_CODE"]) {
				$reponse = "E21";
				return $reponse;
			}
		}
			...
			if ($check_sticker_paired["MASCO_MBRAN_CODE"] != $data_batch["SUBPA_MBRAN_CODE"]) {
				$reponse = "E21";
				return $reponse;
			}
