...
class LabResultController extends Controller
{
    ...
    public function register_test_lab_customer($request)
    {
        ...
        $request["qr_alpha"] = "MXX0000001";
        $request["qr_zeta"] = "MXX0000001";
        ...
        // $server = "http://localhost/cek_ori"; //localhost
        $server = "http://54.169.106.215"; // Legacy Server
        // $server = "http://134.209.124.184"; // Old Server

        $token_registration = curl_post($server."/api/v5/test_lab_register/register",$data);
        if (isset($token_registration["data"])) {
            if (isset($token_registration["data"]["token"])) {

                $request["user_token"] = $token_registration["data"]["token"];
                $this->send_email_after_register($request);

            } else if (isset($token_registration["data"]["data_user"])) {

                $request["customer_email"] = $token_registration["data"]["data_user"]["user_email"];
                $request["customer_phone_number"] = $token_registration["data"]["data_user"]["user_country_phone"].$token_registration["data"]["data_user"]["user_phone"];
                $request["patient"] = $token_registration["data"]["data_user"]["user_first_name"]." ".$token_registration["data"]["data_user"]["user_last_name"];
    ...
    public function send_email_not_register($request)
    {
        ...
        $test_date["TRQRA_EMP_SCAN_TIMESTAMP"] = "2021-04-22 11:10:00";
    ...
    {
        $this->register_test_lab_customer($request->lab_result[0]);
